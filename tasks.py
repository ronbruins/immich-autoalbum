...
def mainexecutor(user_exec, task):
    for iun in user_exec:
        ...
        if task == "libloop":
            libloop(user_exec,rbimmich)
        elif task == "tagloop":
            tagloop(user_exec,rbimmich)
        elif task == "updateloop":
            update_album_dict = update_album_list(user_exec,rbimmich,api_key)
            print(update_album_dict)
    if task == "updateloop":
...
def libloop(user_exec,rbimmich):
    # for iun in user_exec:
    #     init_user = init_users[iun]
    #     api_key = api_keys[init_user]
    #     rbimmich = imclass_V2.ImmichApi(api_key,base_url,init_user,admin_api)
...
def tagloop(user_exec,rbimmich):
    # for iun in user_exec:
    #     #  for iun in "1":
    #     init_user = init_users[iun]
    #     api_key = api_keys[init_user]
...
def update_album_list(user_exec,rbimmich,api_key):
    # update_album_dict = {}
    # for iun in user_exec:
    #     init_user = init_users[iun]
    #     api_key = api_keys[init_user]
